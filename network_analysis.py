...
def modularity(G):
	"Takes a graph object and returns a modularity score: float"
	partition = community.best_partition(G)
	return community.modularity(partition, G)

def calculate_average_neighbor_difference(graph, temp_d_sim_score):
	'''
    Returns the average difference of scores of all neighbors in a graph. 

            Parameters:
                    graph (Networkx Graph): Networkx Graph Object
                    temp_d_sim_score (dict): Dictionary of scores assigned to each node in graph.

            Returns:
                    average_difference (float): average score difference between all neighbors
    '''
	total_difference = 0
	total_neighbors = 0
...
def calculate_homophily_scores(G, neighbor_threshold=.1):
	'''
    Returns homophily score for a graph 

            Parameters:
                    G (Networkx Graph): Networkx Graph Object
                    neighbor_threshold (float): value to group nodes together by

            Returns:
                    homophily_scores (dict): homophily scores of all nodes in G
    '''
	homophily_scores = {}
	for node in G.nodes():
...
def graph_average_homophily(G, neighbor_threshold=.1):
	"Averages the homophily scores of all nodes in G."
...
def network_stats_over_time(G, d_sim_to_avg):
	'''
    Writes different network statistics over time to a CSV file 

            Parameters:
                    G (Networkx Graph): Networkx Graph Object
                    d_sim_to_avg (dict): scores associated with each node
